...
[[assertj-core-3.14.0-]]
[.release-note-item]#`Add `isEmpty` and `isNotEmpty` assertions for files#
...
`extracting` is now able to extract a deeply nested map key, before this improvement extracting a value by key was only supported for Map object under test (but not for a Map fields).
...
assertThat(luke).extracting("name.last", "attributes.side", 
                            "relations.sister", "relations.sister.relations.brother")
                .containsExactly("Skywalker", "light", leia, luke);
----
